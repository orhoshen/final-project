...
/// Service to fetch pre-saved melodies from Firebase or a local JSON for testing
class FirebaseMelodyService {
  // Configuration constants - moved from TODO to proper configuration
  static const String _apiBaseUrl = 'http://localhost:5001/api/melodies';
  static const String _localMelodiesUrl = 'http://localhost:5001/static/melodies.json';
  static const String _firestoreProjectId = 'finalproj-piano-game';
  static const String _firestoreCollectionPath = 'melodies';

  // Cache configuration (in use)
  static const Duration _shortNetworkTimeout = Duration(seconds: 3);
  static const Duration _melodyFetchTimeout = Duration(seconds: 5);

  // MIDI configuration (in use)
  static const int _minMidiNote = 48; // C3
  static const int _maxMidiNote = 84; // C6
  ...
  Future<List<int>> getMelody({int complexity = 4}) async {
    ...
    if (_cachedMelodiesByDifficulty.containsKey(complexity) && _cachedMelodiesByDifficulty[complexity]!.isNotEmpty) {
      final random = Random();
  ...
  Future<void> fetchMelodies() async {
    try {
      ...
      try {
        ...
        final response = await http.get(Uri.parse(_localMelodiesUrl)).timeout(_shortNetworkTimeout);
          ...
          if (data.containsKey('melodies') && data['melodies'] is List) {
            ...
            for (final melody in melodies) {
              if (melody['notes'] is List) {
                ...
                if (notes.isNotEmpty) {
                  // Filter out notes outside the configured MIDI range
                  bool validRange = true;
                  for (final note in notes) {
                    if (note < _minMidiNote || note > _maxMidiNote) {
                      validRange = false;
                      break;
                    }
                  }
                  ...
                  if (validRange) {
                    ...
                    developer.log('Skipped melody with notes outside configured MIDI range');
                  }
                }
              }
            }
      ...
      try {
        final response = await http
            .get(
              Uri.parse(
                'https://firestore.googleapis.com/v1/projects/$_firestoreProjectId/databases/(default)/documents/$_firestoreCollectionPath',
              ),
            )
            .timeout(_melodyFetchTimeout);
              ...
              if (doc.containsKey('fields') &&
                ...
                if (notesArray.containsKey('values')) {
                  ...
                  final notes = notesList
                      .map((note) => note.containsKey('integerValue') ? int.parse(note['integerValue']) : null)
  ...
  Future<void> _fetchLegacyMelodies() async {
    try {
      ...
      final response = await http.get(Uri.parse('$_apiBaseUrl/list')).timeout(_melodyFetchTimeout);
          ...
          for (final melody in melodies) {
            ...
            if (noteSequence.isNotEmpty) {
              ...
              if (validRange) {
                ...
                developer.log('Skipped legacy melody with notes outside configured MIDI range');
              }
            }
          }
  ...
  List<int> _generateFallbackMelody(int complexity) {
    ...
    for (int i = 1; i < length; i++) {
      ...
      // Keep within configured MIDI range
      currentNote = currentNote.clamp(_minMidiNote, _maxMidiNote);
      result.add(currentNote);
    }
