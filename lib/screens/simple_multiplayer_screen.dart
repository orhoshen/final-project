...
/// Simplified multiplayer screen that works directly with Socket.IO
/// without complex provider chains
...
class _SimpleMultiplayerScreenState extends State<SimpleMultiplayerScreen> {
  ...
  Future<void> _checkServerAndConnect() async {
    ...
    bool serverAvailable = await ServerManager.instance.testConnection();
    
    if (!serverAvailable) {
      setState(() {
        _connectionStatus = 'Server offline';
        _gameStatus = 'Cannot connect - server is not running';
      });
      return;
    }
      ...
      if (data['room'] != null) {
        ...
        setState(() {
          ...
          _roomPlayers = (room['players'] as List? ?? [])
              ...
              .toList();
          _isInRoom = true;
          _gameStatus = 'In room $_roomId with ${_roomPlayers.length} players';
        });
      }
    });
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (_roomPlayers.isNotEmpty) ...[
                      const SizedBox(height: 8),
                      Text('Players in room: ${_roomPlayers.join(", ")}'),
                    ],
                  ],
                ),
              ),
